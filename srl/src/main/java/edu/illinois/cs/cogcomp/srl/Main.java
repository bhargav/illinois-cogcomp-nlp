...
public class Main {
    ...
    @CommandDescription(
            description = "Performs the full training & testing sequence for all SRL types",
    ...
    private static void cacheVerbNom(SRLType srlType, TextAnnotationCache taCache) throws Exception {
        ...
        if (srlType == SRLType.Verb)
            ...
            dataHome = properties.getNombankHome();
    ...
    private static void addRequiredViews(IResetableIterator<TextAnnotation> dataset, TextAnnotationCache taCache)
        ...
        while (dataset.hasNext()) {
            ...
            if (parser.equals("Charniak"))
                parserView += ViewNames.PARSE_CHARNIAK;
            if (parser.equals("Berkeley"))
                parserView += ViewNames.PARSE_BERKELEY;
    ...
    @CommandDescription(
            description = "Pre-extracts the features for a specific model and SRL type. "
    ...
    public static void preExtract(String srlType_, String model) throws Exception {
        ...
        String gapStr = "";
        for (int i = 0; i < ((29 - model.length()) / 2); i++)
            gapStr += " ";
        log.info("\n\n\n\n" + "**************************************************\n" + "** "
        ...
        String allDataCacheFile =
                properties.getFeatureCacheFile(srlType, modelToExtract, featureSet, defaultParser,
                        dataset);
        FeatureVectorCacheFile featureCache =
                preExtract(numConsumers, manager, modelToExtract, dataset, allDataCacheFile, false);

        pruneFeatures(numConsumers, manager, modelToExtract, featureCache,
                properties.getPrunedFeatureCacheFile(srlType, modelToExtract, featureSet,
                        defaultParser));

        Lexicon lexicon =
                modelInfo.getLexicon().getPrunedLexicon(manager.getPruneSize(modelToExtract));
        ...
        if (modelToExtract == Models.Identifier) {
            ...
            String devCacheFile =
                    properties.getFeatureCacheFile(srlType, modelToExtract, featureSet,
                            defaultParser, datasetIdentifier);
            preExtract(numConsumers, manager, modelToExtract, datasetIdentifier, devCacheFile,
                    false);
        }
    }

    private static void pruneFeatures(int numConsumers, SRLManager manager, Models modelToExtract,
        ...
        FeatureVectorCacheFile prunedfeatureCache =
                new FeatureVectorCacheFile(cacheFile2, modelToExtract, manager);

        PruningPreExtractor p1 =
                new PruningPreExtractor(manager, modelToExtract, featureCache, prunedfeatureCache,
                        numConsumers);
        p1.run();
        p1.finalize();
    }

    private static FeatureVectorCacheFile preExtract(int numConsumers, SRLManager manager,
            Models modelToExtract, Dataset dataset, String cacheFile, boolean lockLexicon)
            throws Exception {
        if (IOUtils.exists(cacheFile)) {
            log.warn("Old cache file found. Returning it...");
            FeatureVectorCacheFile vectorCacheFile =
            ...
            while (vectorCacheFile.hasNext()) {
                ...
                IFeatureVector cachedFeatureVector =
                        pair.getFirst().getCachedFeatureVector(modelToExtract);
                int length = cachedFeatureVector.getNumActiveFeatures();
                for (int i = 0; i < length; i++) {
                    manager.getModelInfo(modelToExtract).getLexicon()
                            .countFeature(cachedFeatureVector.getIdx(i));
                }
            }
    ...
    public static void train(String srlType_, String model_) throws Exception {
        ...
        String gapStr = "";
        for (int i = 0; i < ((36 - model_.length()) / 2); i++)
            gapStr += " ";
        log.info("\n\n\n\n" + "**************************************************\n" + "** "
        ...
        String featureSet = "" + modelInfo.featureManifest.getIncludedFeatures().hashCode();
        String cacheFile =
                properties.getPrunedFeatureCacheFile(srlType, model, featureSet, defaultParser);
    ...
    private static void tuneIdentifier(String srlType_) throws Exception {
        ...
        String cacheFile =
                properties.getFeatureCacheFile(srlType, Models.Identifier, featureSet,
                        defaultParser, dataset);

        FeatureVectorCacheFile cache =
                new FeatureVectorCacheFile(cacheFile, Models.Identifier, manager);
    ...
    public static void evaluate(String srlType_) throws Exception {
        ...
        if (outDir != null) {
            ...
            String outputFilePrefix =
                    outDir + "/" + srlType + "." + manager.defaultParser + "."
                            + new Random().nextInt();
        ...
        while (dataset.hasNext()) {
            ...
            if (count % 1000 == 0) {
                ...
                log.info(count + " sentences done. Took " + (end - start) + "ms, " + "F1 so far = "
                        + tester.getMacroF1());
            }
        }
        long end = System.currentTimeMillis();
